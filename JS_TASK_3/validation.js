...
form.addEventListener("submit", (e) => {
  ...
  if (basicValid && eduValid && workValid && skillValid && langValid && prefValid) {
    form.submit();
  }
});
...
function language_validation() {
  ...
  languageInputs.forEach((input) => {
    ...
    if (input.checked) {
      ...
      const readCheckbox = form.querySelector(`input[name="lang[${lang}][read]"]`);
      const writeCheckbox = form.querySelector(`input[name="lang[${lang}][write]"]`);
      const speakCheckbox = form.querySelector(`input[name="lang[${lang}][speak]"]`);
